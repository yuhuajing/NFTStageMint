...
import "https://github.com/LayerZero-Labs/solidity-examples/blob/main/contracts/token/onft721/ONFT721Core.sol";
import "@openzeppelin/contracts/utils/cryptography/SignatureChecker.sol";
import "./ISRCNFT.sol";
import {Math} from "@openzeppelin/contracts/utils/math/Math.sol";

contract ONFT721A is ONFT721Core, ERC721A, ERC721A__IERC721Receiver, ISRCNFT {
    ...
    mapping(address => mapping(uint256 => uint256)) private stageMaxSupply;
        ...
        returns (uint32, uint64)
    {
        ...
        require((!ridvalue[requestId]), "HAS_USED");
    ...
    function _mintInternal(
        ...
        (bool success, uint256 mintnum) = Math.tryMod(uint256(qty), 100);
        require(success);
        (bool _success, uint256 MaxMum) = Math.tryDiv(uint256(qty), 10000);
        require(_success);

        require(
            (totalSupply() + mintnum <= maxSupply),
            "exceed the max supply"
        );
        if (!mintable) revert NotMintable();

        // uint64 stageTimestamp = uint64(block.timestamp);
        _assertValidTimestamp(stageTimestamp);

        uint256 activeStage = getActiveStageFromTimestamp(stageTimestamp);

        require(
            (stageMaxSupply[msg.sender][activeStage] + mintnum < MaxMum),
            "Over_Mint"
        );
        stageMaxSupply[msg.sender][activeStage] += mintnum;
        if (
            stageMintedCounts[activeStage] + mintnum >
        ...
        if (msg.sender != owner()) {
            ...
            if (msg.value < price * mintnum) revert NotEnoughValue();
        }

        stageMintedCounts[activeStage] += mintnum;
        _safeMint(to, mintnum);
    }

    function burn(uint256 tokenId) public virtual {
        require((checkStakeEnd(tokenId)), "TokenId_IN_STAKE");
        _burn(tokenId, true);
    }

    function stake(uint256 tokenId, uint24 _days) public virtual {
        require((ownerOf(tokenId) == msg.sender), "NOT_TOKENID_OWNER");
    ...
    function transferFrom(
        ...
        require((checkStakeEnd(_tokenId)), "TokenId_IN_STAKE");
    ...
    function _assertValidTimestamp(uint64 timestamp) internal view {
        uint256 originalSize = mintStages.length;
        require((originalSize != 0), "NO_STAGE_SET");
        _assertValidStartAndEndTimestamp(
            mintStages[originalSize - 1].endTimeUnixSeconds,
            timestamp
        );
        if (timestamp < mintStages[0].startTimeUnixSeconds)
